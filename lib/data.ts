export const items = [
  {
    id: 1,
    image: "/image.png",
  },
  {
    id: 2,
    image: "/image.png",
  },
  {
    id: 3,
    image: "/image.png",
  },
  {
    id: 4,
    image: "/image.png",
  },
  {
    id: 5,
    image: "/image.png",
  },
  {
    id: 6,
    image: "/image.png",
  },
  {
    id: 7,
    image: "/image.png",
  },
  {
    id: 8,
    image: "/image.png",
  },
];

export const data = [
  {
    id: "1",
    map: "/floormap.png",
    text: "Nayuta概要説明欄。ここに何か文章を掲載します。ここに何か文章を掲載します。ここに何か文章を掲載します。",
    cards: [
      {
        id: "11",
        icon: "/icon.png",
        title: "Facility name  (List element)",
        discription:
          "Nayuta概要説明欄。ここに何か文章を掲載します。<br>ここに何か文章を掲載します。ここに何か文章を掲載します。",
      },
      {
        id: "12",
        icon: "/icon.png",
        title: "Facility name  (List element)",
        discription:
          "Nayuta概要説明欄。ここに何か文章を掲載します。<br>ここに何か文章を掲載します。ここに何か文章を掲載します。",
      },
      {
        id: "13",
        icon: "/icon.png",
        title: "Facility name  (List element)",
        discription:
          "Nayuta概要説明欄。ここに何か文章を掲載します。<br>ここに何か文章を掲載します。ここに何か文章を掲載します。",
      },
      {
        id: "14",
        icon: "/icon.png",
        title: "Facility name  (List element)",
        discription:
          "Nayuta概要説明欄。ここに何か文章を掲載します。<br>ここに何か文章を掲載します。ここに何か文章を掲載します。",
      },
      {
        id: "15",
        icon: "/icon.png",
        title: "Facility name  (List element)",
        discription:
          "Nayuta概要説明欄。ここに何か文章を掲載します。<br>ここに何か文章を掲載します。ここに何か文章を掲載します。",
      },
      {
        id: "16",
        icon: "/icon.png",
        title: "Facility name  (List element)",
        discription:
          "Nayuta概要説明欄。ここに何か文章を掲載します。<br>ここに何か文章を掲載します。ここに何か文章を掲載します。",
      },
      {
        id: "17",
        icon: "/icon.png",
        title: "Facility name  (List element)",
        discription:
          "Nayuta概要説明欄。ここに何か文章を掲載します。<br>ここに何か文章を掲載します。ここに何か文章を掲載します。",
      },
    ],
  },
  {
    id: "2",
    map: "/floormap.png",
    text: "Nayuta概要説明欄。ここに何か文章を掲載します。ここに何か文章を掲載します。ここに何か文章を掲載します。",
    cards: [
      {
        id: "21",
        icon: "/icon.png",
        title: "Facility name  (List element)",
        discription:
          "Nayuta概要説明欄。ここに何か文章を掲載します。<br>ここに何か文章を掲載します。ここに何か文章を掲載します。",
      },
      {
        id: "22",
        icon: "/icon.png",
        title: "Facility name  (List element)",
        discription:
          "Nayuta概要説明欄。ここに何か文章を掲載します。<br>ここに何か文章を掲載します。ここに何か文章を掲載します。",
      },
      {
        id: "23",
        icon: "/icon.png",
        title: "Facility name  (List element)",
        discription:
          "Nayuta概要説明欄。ここに何か文章を掲載します。<br>ここに何か文章を掲載します。ここに何か文章を掲載します。",
      },
      {
        id: "24",
        icon: "/icon.png",
        title: "Facility name  (List element)",
        discription:
          "Nayuta概要説明欄。ここに何か文章を掲載します。<br>ここに何か文章を掲載します。ここに何か文章を掲載します。",
      },
      {
        id: "25",
        icon: "/icon.png",
        title: "Facility name  (List element)",
        discription:
          "Nayuta概要説明欄。ここに何か文章を掲載します。<br>ここに何か文章を掲載します。ここに何か文章を掲載します。",
      },
    ],
  },
];

export const images = [
  "/menu/brooke-lark-RrzeCGujVfU-unsplash.jpg",
  "/menu/davey-gravy-4WPcz_5RVMk-unsplash.jpg",
  "/menu/khloe-arledge-V7hibs9xhe4-unsplash.jpg",
  "/menu/luisa-brimble-vIm26fn_QKg-unsplash.jpg",
];

type Data = {
  type: "left" | "right";
  path: string;
  title?: string;
  subTitle?: string;
  text: string;
};

export const datas = [
  {
    type: "left",
    path: "/nattu.jpg",
    title: "産直マーケットで取り扱う商品の紹介",
    subTitle: "2行目も必要な場合の行",
    text: "コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。",
  },
  {
    type: "right",
    path: "/nattu.jpg",
    title: "産直マーケットで取り扱う商品の紹介",
    subTitle: "2行目も必要な場合の行",
    text: "コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。",
  },
  {
    type: "right",
    path: "/nattu.jpg",
    title: "産直マーケットで取り扱う商品の紹介",
    subTitle: "2行目も必要な場合の行",
    text: "コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。",
  },
  {
    type: "right",
    path: "/nattu.jpg",
    title: "産直マーケットで取り扱う商品の紹介",
    subTitle: "2行目も必要な場合の行",
    text: "コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。",
  },
  {
    type: "left",
    path: "/nattu.jpg",
    title: "産直マーケットで取り扱う商品の紹介",
    subTitle: "2行目も必要な場合の行",
    text: "コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。",
  },
  {
    type: "left",
    path: "/nattu.jpg",
    title: "産直マーケットで取り扱う商品の紹介",
    subTitle: "2行目も必要な場合の行",
    text: "コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。",
  },
  {
    type: "left",
    path: "/nattu.jpg",
    title: "産直マーケットで取り扱う商品の紹介",
    subTitle: "2行目も必要な場合の行",
    text: "コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。",
  },
] as Data[];

export const bannersData = [
  {
    id: 1,
    src: "/japanese-food.jpg",
    path: "/",
    title: "日本食レストラン",
    text: "コンテンツを紹介する文章をここに掲載します。コンテンツを紹介する文章をここに掲載します",
  },
  {
    id: 2,
    src: "/vegan-restaurant.jpg",
    path: "/",
    title: "ヴィーガンレストラン",
    text: "コンテンツを紹介する文章をここに掲載します。コンテンツを紹介する文章をここに掲載します",
  },
  {
    id: 3,
    src: "/juice-bar.jpg",
    path: "/",
    title: "ジュースバー",
    text: "コンテンツを紹介する文章をここに掲載します。コンテンツを紹介する文章をここに掲載します",
  },
  {
    id: 4,
    src: "/nattu.jpg",
    path: "/",
    title: "量り売り&直売所量り売り&直売所",
    text: "コンテンツを紹介する文章をここに掲載します。コンテンツを紹介する文章をここに掲載します",
  },
  {
    id: 5,
    src: "/ec-1.jpg",
    path: "/",
    title: "セレクトショップ&ECサイト",
    text: "コンテンツを紹介する文章をここに掲載します。コンテンツを紹介する文章をここに掲載します",
  },
  {
    id: 6,
    src: "/caffe.jpg",
    path: "/",
    title: "カフェカフェ",
    text: "コンテンツを紹介する文章をここに掲載します。コンテンツを紹介する文章をここに掲載します",
  },
  {
    id: 7,
    src: "/rental-space.jpg",
    path: "/",
    title: "レンタルスペース",
    text: "コンテンツを紹介する文章をここに掲載します。コンテンツを紹介する文章をここに掲載します",
  },
  {
    id: 8,
    src: "/hotel.jpg",
    path: "/",
    title: "ホテル",
    text: "コンテンツを紹介する文章をここに掲載します。コンテンツを紹介する文章をここに掲載します",
  },
];

export const DUMMY_CONCERT = [
  {
    id: "1",
    srcImg: "/concert/live_house_1.jpg",
    altImg: "image of a live",
    titleArticle:
      "1-コンサートホールの使い方についてサブタイトルを掲載します。サブタイトルを掲載します。",
    textArticle:
      "コンサートホールの使用方法概要説明欄。ここに何か文章を掲載します。ここに何か文章を掲載します。ここに何か文章を掲載します。<br/><br/> ここに何か文章を掲載します。ここに何か文章を掲載します。",
  },
  {
    id: "2",
    srcImg: "/concert/live_house_2.jpg",
    altImg: "image of a singer",
    titleArticle:
      "2-コンサートホールの使い方についてサブタイトルを掲載します。サブタイトルを掲載します。",
    textArticle:
      "コンサートホールの使用方法概要説明欄。ここに何か文章を掲載します。ここに何か文章を掲載します。ここに何か文章を掲載します。<br/><br/> ここに何か文章を掲載します。ここに何か文章を掲載します。",
  },
  {
    id: "3",
    srcImg: "/concert/live_house_3.jpg",
    altImg: "image of guitars",
    titleArticle:
      "3-コンサートホールの使い方についてサブタイトルを掲載します。サブタイトルを掲載します。",
    textArticle:
      "コンサートホールの使用方法概要説明欄。ここに何か文章を掲載します。ここに何か文章を掲載します。ここに何か文章を掲載します。<br/><br/> ここに何か文章を掲載します。ここに何か文章を掲載します。",
  },
];

export const DUMMY_CONCERT_INFOS = [
  {
    id: "1",
    srcImg: "/concert/live_house_1.jpg",
    altImg: "image of a live",
    titleArticle:
      "1-機材名称・型番などの情報を掲載します。機材名称・型番などの情報を掲載します。機材名称・型番などの情報を掲載します。",
    textArticle:
      "機材の状態などを記載します。コンサートホールの使用方法概要説明欄。ここに何か文章を掲載します。ここに何か文章を掲載します。ここに何か文章を掲載します。<br/>ここに何か文章を掲載します。ここに何か文章を掲載します。",
  },
  {
    id: "2",
    srcImg: "/concert/live_house_2.jpg",
    altImg: "image of a singer",
    titleArticle:
      "2-機材名称・型番などの情報を掲載します。機材名称・型番などの情報を掲載します。機材名称・型番などの情報を掲載します。",
    textArticle:
      "機材の状態などを記載します。コンサートホールの使用方法概要説明欄。ここに何か文章を掲載します。ここに何か文章を掲載します。ここに何か文章を掲載します。<br/>ここに何か文章を掲載します。ここに何か文章を掲載します。",
  },
  {
    id: "3",
    srcImg: "/concert/live_house_3.jpg",
    altImg: "image of guitars",
    titleArticle:
      "3-機材名称・型番などの情報を掲載します。機材名称・型番などの情報を掲載します。機材名称・型番などの情報を掲載します。",
    textArticle:
      "機材の状態などを記載します。コンサートホールの使用方法概要説明欄。ここに何か文章を掲載します。ここに何か文章を掲載します。ここに何か文章を掲載します。<br/>ここに何か文章を掲載します。ここに何か文章を掲載します。",
  },
  {
    id: "4",
    srcImg: "/concert/live_house_1.jpg",
    altImg: "image of a live",
    titleArticle:
      "4-機材名称・型番などの情報を掲載します。機材名称・型番などの情報を掲載します。機材名称・型番などの情報を掲載します。",
    textArticle:
      "機材の状態などを記載します。コンサートホールの使用方法概要説明欄。ここに何か文章を掲載します。ここに何か文章を掲載します。ここに何か文章を掲載します。<br/>ここに何か文章を掲載します。ここに何か文章を掲載します。",
  },
  {
    id: "5",
    srcImg: "/concert/live_house_2.jpg",
    altImg: "image of a singer",
    titleArticle:
      "5-機材名称・型番などの情報を掲載します。機材名称・型番などの情報を掲載します。機材名称・型番などの情報を掲載します。",
    textArticle:
      "機材の状態などを記載します。コンサートホールの使用方法概要説明欄。ここに何か文章を掲載します。ここに何か文章を掲載します。ここに何か文章を掲載します。<br/>ここに何か文章を掲載します。ここに何か文章を掲載します。",
  },
  {
    id: "6",
    srcImg: "/concert/live_house_3.jpg",
    altImg: "image of guitars",
    titleArticle:
      "6-機材名称・型番などの情報を掲載します。機材名称・型番などの情報を掲載します。機材名称・型番などの情報を掲載します。",
    textArticle:
      "機材の状態などを記載します。コンサートホールの使用方法概要説明欄。ここに何か文章を掲載します。ここに何か文章を掲載します。ここに何か文章を掲載します。<br/>ここに何か文章を掲載します。ここに何か文章を掲載します。",
  },
  {
    id: "7",
    srcImg: "/concert/live_house_1.jpg",
    altImg: "image of a live",
    titleArticle:
      "7-機材名称・型番などの情報を掲載します。機材名称・型番などの情報を掲載します。機材名称・型番などの情報を掲載します。",
    textArticle:
      "機材の状態などを記載します。コンサートホールの使用方法概要説明欄。ここに何か文章を掲載します。ここに何か文章を掲載します。ここに何か文章を掲載します。<br/>ここに何か文章を掲載します。ここに何か文章を掲載します。",
  },
  {
    id: "8",
    srcImg: "/concert/live_house_2.jpg",
    altImg: "image of a singer",
    titleArticle:
      "8-機材名称・型番などの情報を掲載します。機材名称・型番などの情報を掲載します。機材名称・型番などの情報を掲載します。",
    textArticle:
      "機材の状態などを記載します。コンサートホールの使用方法概要説明欄。ここに何か文章を掲載します。ここに何か文章を掲載します。ここに何か文章を掲載します。<br/>ここに何か文章を掲載します。ここに何か文章を掲載します。",
  },
  {
    id: "9",
    srcImg: "/concert/live_house_2.jpg",
    altImg: "image of a singer",
    titleArticle:
      "機材名称・型番などの情報を掲載します。機材名称・型番などの情報を掲載します。機材名称・型番などの情報を掲載します。",
    textArticle:
      "機材の状態などを記載します。コンサートホールの使用方法概要説明欄。ここに何か文章を掲載します。ここに何か文章を掲載します。ここに何か文章を掲載します。<br/>ここに何か文章を掲載します。ここに何か文章を掲載します。",
  },
  {
    id: "10",
    srcImg: "/concert/live_house_3.jpg",
    altImg: "image of guitars",
    titleArticle:
      "機材名称・型番などの情報を掲載します。機材名称・型番などの情報を掲載します。機材名称・型番などの情報を掲載します。",
    textArticle:
      "機材の状態などを記載します。コンサートホールの使用方法概要説明欄。ここに何か文章を掲載します。ここに何か文章を掲載します。ここに何か文章を掲載します。<br/>ここに何か文章を掲載します。ここに何か文章を掲載します。",
  },
  {
    id: "11",
    srcImg: "/concert/live_house_1.jpg",
    altImg: "image of a live",
    titleArticle:
      "機材名称・型番などの情報を掲載します。機材名称・型番などの情報を掲載します。機材名称・型番などの情報を掲載します。",
    textArticle:
      "機材の状態などを記載します。コンサートホールの使用方法概要説明欄。ここに何か文章を掲載します。ここに何か文章を掲載します。ここに何か文章を掲載します。<br/>ここに何か文章を掲載します。ここに何か文章を掲載します。",
  },
  {
    id: "12",
    srcImg: "/concert/live_house_2.jpg",
    altImg: "image of a singer",
    titleArticle:
      "機材名称・型番などの情報を掲載します。機材名称・型番などの情報を掲載します。機材名称・型番などの情報を掲載します。",
    textArticle:
      "機材の状態などを記載します。コンサートホールの使用方法概要説明欄。ここに何か文章を掲載します。ここに何か文章を掲載します。ここに何か文章を掲載します。<br/>ここに何か文章を掲載します。ここに何か文章を掲載します。",
  },
];

export const OnigiriMenuItemsDatas = [
  {
    id: "1",
    srcImg: "/bar/onigiri/onigiri-1.jpeg",
    altImg: "onigiri simple",
    title: "紹介メニュー名称—————掲載———————スペース 1",
    price: "0,000",
    allergy: [
      {
        src: "/bar/eco-icon.svg",
        alt: "peanut",
      },
      {
        src: "/bar/eco-icon.svg",
        alt: "egg",
      },
      {
        src: "/bar/eco-icon.svg",
        alt: "banane",
      },
      {
        src: "/bar/eco-icon.svg",
        alt: "peanut",
      },
      {
        src: "/bar/eco-icon.svg",
        alt: "egg",
      },
      {
        src: "/bar/eco-icon.svg",
        alt: "banane",
      },
      {
        src: "/bar/eco-icon.svg",
        alt: "peanut",
      },
      {
        src: "/bar/eco-icon.svg",
        alt: "egg",
      },
      {
        src: "/bar/eco-icon.svg",
        alt: "banane",
      },
      {
        src: "/bar/eco-icon.svg",
        alt: "peanut",
      },
      {
        src: "/bar/eco-icon.svg",
        alt: "egg",
      },
      {
        src: "/bar/eco-icon.svg",
        alt: "banane",
      },
    ],
    description:
      "コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。<br/><br/>コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。",
  },
  {
    id: "2",
    srcImg: "/bar/onigiri/onigiri-2.jpeg",
    altImg: "onigiri simple",
    title: "紹介メニュー名称—————掲載———————スペース 2",
    price: "0,000",
    allergy: [
      {
        src: "/bar/eco-icon.svg",
        alt: "peanut",
      },
      {
        src: "/bar/eco-icon.svg",
        alt: "egg",
      },
      {
        src: "/bar/eco-icon.svg",
        alt: "banane",
      },
    ],
    description:
      "コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。<br/><br/>コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。",
  },
  {
    id: "3",
    srcImg: "/bar/onigiri/onigiri-3.jpeg",
    altImg: "onigiri simple",
    title: "紹介メニュー名称—————掲載———————スペース 3",
    price: "0,000",
    allergy: [
      {
        src: "/bar/eco-icon.svg",
        alt: "peanut",
      },
      {
        src: "/bar/eco-icon.svg",
        alt: "egg",
      },
      {
        src: "/bar/eco-icon.svg",
        alt: "banane",
      },
    ],
    description:
      "コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。<br/><br/>コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。",
  },
];

export const JuiceMenuItemsDatas = [
  {
    id: "11",
    srcImg: "/bar/juice/juice-1.jpg",
    altImg: "juice simple",
    title: "紹介メニュー名称—————掲載———————スペース 1",
    price: "0,000",
    allergy: [
      {
        src: "/bar/eco-icon.svg",
        alt: "peanut",
      },
      {
        src: "/bar/eco-icon.svg",
        alt: "egg",
      },
      {
        src: "/bar/eco-icon.svg",
        alt: "banane",
      },
    ],
    description:
      "コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。<br/><br/>コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。",
  },
  {
    id: "12",
    srcImg: "/bar/juice/juice-2.jpg",
    altImg: "juice simple",
    title: "紹介メニュー名称—————掲載———————スペース 2",
    price: "0,000",
    allergy: [
      {
        src: "/bar/eco-icon.svg",
        alt: "peanut",
      },
      {
        src: "/bar/eco-icon.svg",
        alt: "egg",
      },
      {
        src: "/bar/eco-icon.svg",
        alt: "banane",
      },
    ],
    description:
      "コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。<br/><br/>コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。",
  },
  {
    id: "13",
    srcImg: "/bar/juice/juice-3.jpg",
    altImg: "juice simple",
    title: "紹介メニュー名称—————掲載———————スペース 3",
    price: "0,000",
    allergy: [
      {
        src: "/bar/eco-icon.svg",
        alt: "peanut",
      },
      {
        src: "/bar/eco-icon.svg",
        alt: "egg",
      },
      {
        src: "/bar/eco-icon.svg",
        alt: "banane",
      },
    ],
    description:
      "コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。<br/><br/>コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。",
  },
];

export const CaffeMenuItemsDatas = [
  {
    id: "1",
    srcImg: "/cafe/menu/cafe1.jpg",
    altImg: "juice simple",
    title: "紹介メニュー名称—————掲載———————スペース 1",
    price: "0,000",
    allergy: [
      {
        src: "/bar/eco-icon.svg",
        alt: "peanut",
      },
      {
        src: "/bar/eco-icon.svg",
        alt: "egg",
      },
      {
        src: "/bar/eco-icon.svg",
        alt: "banane",
      },
    ],
    description:
      "コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。<br/><br/>コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。",
  },
  {
    id: "2",
    srcImg: "/cafe/menu/cafe2.jpg",
    altImg: "juice simple",
    title: "紹介メニュー名称—————掲載———————スペース 2",
    price: "0,000",
    allergy: [
      {
        src: "/bar/eco-icon.svg",
        alt: "peanut",
      },
      {
        src: "/bar/eco-icon.svg",
        alt: "egg",
      },
      {
        src: "/bar/eco-icon.svg",
        alt: "banane",
      },
    ],
    description:
      "コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。<br/><br/>コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。",
  },
  {
    id: "3",
    srcImg: "/cafe/menu/cafe3.jpg",
    altImg: "juice simple",
    title: "紹介メニュー名称—————掲載———————スペース 3",
    price: "0,000",
    allergy: [
      {
        src: "/bar/eco-icon.svg",
        alt: "peanut",
      },
      {
        src: "/bar/eco-icon.svg",
        alt: "egg",
      },
      {
        src: "/bar/eco-icon.svg",
        alt: "banane",
      },
    ],
    description:
      "コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。<br/><br/>コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。",
  },
  {
    id: "4",
    srcImg: "/cafe/menu/cafe1.jpg",
    altImg: "juice simple",
    title: "紹介メニュー名称—————掲載———————スペース 1",
    price: "0,000",
    allergy: [
      {
        src: "/bar/eco-icon.svg",
        alt: "peanut",
      },
      {
        src: "/bar/eco-icon.svg",
        alt: "egg",
      },
      {
        src: "/bar/eco-icon.svg",
        alt: "banane",
      },
    ],
    description:
      "コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。<br/><br/>コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。",
  },
  {
    id: "5",
    srcImg: "/cafe/menu/cafe2.jpg",
    altImg: "juice simple",
    title: "紹介メニュー名称—————掲載———————スペース 2",
    price: "0,000",
    allergy: [
      {
        src: "/bar/eco-icon.svg",
        alt: "peanut",
      },
      {
        src: "/bar/eco-icon.svg",
        alt: "egg",
      },
      {
        src: "/bar/eco-icon.svg",
        alt: "banane",
      },
    ],
    description:
      "コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。<br/><br/>コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。",
  },
  {
    id: "6",
    srcImg: "/cafe/menu/cafe3.jpg",
    altImg: "juice simple",
    title: "紹介メニュー名称—————掲載———————スペース 3",
    price: "0,000",
    allergy: [
      {
        src: "/bar/eco-icon.svg",
        alt: "peanut",
      },
      {
        src: "/bar/eco-icon.svg",
        alt: "egg",
      },
      {
        src: "/bar/eco-icon.svg",
        alt: "banane",
      },
    ],
    description:
      "コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。<br/><br/>コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。コンセプトやキャッチコピーなどを掲載します。",
  },
];

export const DUMMY_NEWS = [
  {
    id: "news1",
    timestamp: "2022.02.02",
    text: "お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。",
  },
  {
    id: "news2",
    timestamp: "2022.02.02",
    text: "お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。",
  },
  {
    id: "news3",
    timestamp: "2022.02.02",
    text: "お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。",
  },
  {
    id: "news4",
    timestamp: "2022.02.02",
    text: "お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。",
  },
  {
    id: "news5",
    timestamp: "2022.02.02",
    text: "お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。",
  },
  {
    id: "news6",
    timestamp: "2022.02.02",
    text: "お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。",
  },
  {
    id: "news7",
    timestamp: "2022.02.02",
    text: "お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。",
  },
  {
    id: "news8",
    timestamp: "2022.02.02",
    text: "お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。",
  },
  {
    id: "news9",
    timestamp: "2022.02.02",
    text: "お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。",
  },
  {
    id: "news10",
    timestamp: "2022.02.02",
    text: "お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。",
  },
  {
    id: "news11",
    timestamp: "2022.02.02",
    text: "お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。",
  },
  {
    id: "news12",
    timestamp: "2022.02.02",
    text: "お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。",
  },
  {
    id: "news13",
    timestamp: "2022.02.02",
    text: "お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。",
  },
  {
    id: "news14",
    timestamp: "2022.02.02",
    text: "お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。",
  },
  {
    id: "news15",
    timestamp: "2022.02.02",
    text: "お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。",
  },
  {
    id: "news16",
    timestamp: "2022.02.02",
    text: "お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。お知らせの文章をここに掲載します。",
  },
];

export const faqData = [
  {
    id: "1",
    title:
      "質問タイトル質問事項 サブタイトル フォントサイズ24px  font-size: 24px;  質問タイトル質問事項 サブタイトル フォントサイズ24px  font-size: 24px;",
    text: "回答内容 回答事項 標準文章用テキストサイズ18px font-size: 18px; for text 標準文章用テキストサイズ18px font-size: 18px; for text 標準文章用テキストサイズ18px font-size: 18px; for text. 回答内容 回答事項 標準文章用テキストサイズ18px font-size: 18px; for text 標準文章用テキストサイズ18px font-size: 18px; for text 標準文章用テキストサイズ18px font-size: 18px; for text. 回答内容 回答事項 標準文章用テキストサイズ18px font-size: 18px; for text 標準文章用テキストサイズ18px font-size: 18px; for text 標準文章用テキストサイズ18px font-size: 18px; for text. 回答内容 回答事項 標準文章用テキストサイズ18px font-size: 18px; for text 標準文章用テキストサイズ18px font-size: 18px; for text 標準文章用テキストサイズ18px font-size: 18px; for text. 回答内容 回答事項 標準文章用テキストサイズ18px font-size: 18px; for text 標準文章用テキストサイズ18px font-size: 18px; for text 標準文章用テキストサイズ18px font-size: 18px; for text. ",
  },
  {
    id: "2",
    title:
      "質問タイトル質問事項 サブタイトル フォントサイズ24px  font-size: 24px;  質問タイトル質問事項 サブタイトル フォントサイズ24px  font-size: 24px;",
    text: "回答内容 回答事項 標準文章用テキストサイズ18px font-size: 18px; for text 標準文章用テキストサイズ18px font-size: 18px; for text 標準文章用テキストサイズ18px font-size: 18px; for text. 回答内容 回答事項 標準文章用テキストサイズ18px font-size: 18px; for text 標準文章用テキストサイズ18px font-size: 18px; for text 標準文章用テキストサイズ18px font-size: 18px; for text. 回答内容 回答事項 標準文章用テキストサイズ18px font-size: 18px; for text 標準文章用テキストサイズ18px font-size: 18px; for text 標準文章用テキストサイズ18px font-size: 18px; for text. 回答内容 回答事項 標準文章用テキストサイズ18px font-size: 18px; for text 標準文章用テキストサイズ18px font-size: 18px; for text 標準文章用テキストサイズ18px font-size: 18px; for text. 回答内容 回答事項 標準文章用テキストサイズ18px font-size: 18px; for text 標準文章用テキストサイズ18px font-size: 18px; for text 標準文章用テキストサイズ18px font-size: 18px; for text. ",
  },
  {
    id: "3",
    title:
      "質問タイトル質問事項 サブタイトル フォントサイズ24px  font-size: 24px;  質問タイトル質問事項 サブタイトル フォントサイズ24px  font-size: 24px;",
    text: "回答内容 回答事項 標準文章用テキストサイズ18px font-size: 18px; for text 標準文章用テキストサイズ18px font-size: 18px; for text 標準文章用テキストサイズ18px font-size: 18px; for text. 回答内容 回答事項 標準文章用テキストサイズ18px font-size: 18px; for text 標準文章用テキストサイズ18px font-size: 18px; for text 標準文章用テキストサイズ18px font-size: 18px; for text. 回答内容 回答事項 標準文章用テキストサイズ18px font-size: 18px; for text 標準文章用テキストサイズ18px font-size: 18px; for text 標準文章用テキストサイズ18px font-size: 18px; for text. 回答内容 回答事項 標準文章用テキストサイズ18px font-size: 18px; for text 標準文章用テキストサイズ18px font-size: 18px; for text 標準文章用テキストサイズ18px font-size: 18px; for text. 回答内容 回答事項 標準文章用テキストサイズ18px font-size: 18px; for text 標準文章用テキストサイズ18px font-size: 18px; for text 標準文章用テキストサイズ18px font-size: 18px; for text. ",
  },
  {
    id: "4",
    title:
      "質問タイトル質問事項 サブタイトル フォントサイズ24px  font-size: 24px;  質問タイトル質問事項 サブタイトル フォントサイズ24px  font-size: 24px;",
    text: "回答内容 回答事項 標準文章用テキストサイズ18px font-size: 18px; for text 標準文章用テキストサイズ18px font-size: 18px; for text 標準文章用テキストサイズ18px font-size: 18px; for text. 回答内容 回答事項 標準文章用テキストサイズ18px font-size: 18px; for text 標準文章用テキストサイズ18px font-size: 18px; for text 標準文章用テキストサイズ18px font-size: 18px; for text. 回答内容 回答事項 標準文章用テキストサイズ18px font-size: 18px; for text 標準文章用テキストサイズ18px font-size: 18px; for text 標準文章用テキストサイズ18px font-size: 18px; for text. 回答内容 回答事項 標準文章用テキストサイズ18px font-size: 18px; for text 標準文章用テキストサイズ18px font-size: 18px; for text 標準文章用テキストサイズ18px font-size: 18px; for text. 回答内容 回答事項 標準文章用テキストサイズ18px font-size: 18px; for text 標準文章用テキストサイズ18px font-size: 18px; for text 標準文章用テキストサイズ18px font-size: 18px; for text. ",
  },
  {
    id: "5",
    title:
      "質問タイトル質問事項 サブタイトル フォントサイズ24px  font-size: 24px;  質問タイトル質問事項 サブタイトル フォントサイズ24px  font-size: 24px;",
    text: "回答内容 回答事項 標準文章用テキストサイズ18px font-size: 18px; for text 標準文章用テキストサイズ18px font-size: 18px; for text 標準文章用テキストサイズ18px font-size: 18px; for text. 回答内容 回答事項 標準文章用テキストサイズ18px font-size: 18px; for text 標準文章用テキストサイズ18px font-size: 18px; for text 標準文章用テキストサイズ18px font-size: 18px; for text. 回答内容 回答事項 標準文章用テキストサイズ18px font-size: 18px; for text 標準文章用テキストサイズ18px font-size: 18px; for text 標準文章用テキストサイズ18px font-size: 18px; for text. 回答内容 回答事項 標準文章用テキストサイズ18px font-size: 18px; for text 標準文章用テキストサイズ18px font-size: 18px; for text 標準文章用テキストサイズ18px font-size: 18px; for text. 回答内容 回答事項 標準文章用テキストサイズ18px font-size: 18px; for text 標準文章用テキストサイズ18px font-size: 18px; for text 標準文章用テキストサイズ18px font-size: 18px; for text. ",
  },
];
